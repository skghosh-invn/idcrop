...
class IdCrop {
  ...
  init() {
    ...
    if (this.config.numPoints === Infinity && this.config.closeButtonSelector) {
      var closeButton = "";
      closeButton = document.querySelector(this.config.closeButtonSelector)
      closeButton.addEventListener("click", function() {
        if(self.getPoints().length > 0) { 
          self.startCroppingPolygon();
        }
        else {
          var crop = new CustomEvent("crop", { detail: self.originalImage  });
    ...
    if (!c.toolbarArea && this.config.allowUpload) {
      ...
      var hintText = document.createTextNode(" or drag and drop \n SVG, PNG, JPG or PDF (max. 800x400px)");
      var fakeInput = document.createElement("a");
      var fakeInputText = document.createTextNode("Click to upload ");
  ...
  drawPreview(self) {
    if (
      ...
      if( self.config.containers.previewArea){
